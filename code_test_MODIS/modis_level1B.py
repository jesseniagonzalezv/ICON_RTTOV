...
def scale_image(image, x, y,along_track=2030, cross_trak=1354):
...
#     reflectance_RGB[:,:,2]= 0.5*(reflectance_RGB[:,:,0]+reflectance_RGB[:,:,1])
    ...
    norme = 0.8 #0.2 # 0,4factor to increase the brightness ]0,1] https://moonbooks.org/Codes/Plot-MODIS-granule-RGB-image-using-python/
...
########3
    ...
    #CS = plt.imshow(np.fliplr(rgb_out)*2.0, interpolation='nearest', origin='lower')
    CS = plt.imshow(np.fliplr(z_color_enh), interpolation='nearest', origin='lower')
    ...
    #cax=fig.colorbar(CS)
    ax.set_title('MODIS RGB reflectance', fontsize=14)
    ...
    plt.savefig(out_file+"/modis_rgb.png")#, bbox_inches='tight', dpi=100)
    plt.close()
